...
function PageContents({ page, onBookingOpen }: { page: Page; onBookingOpen?: () => void }) {
  return <>
    ...
    {page === "Booking" && <><div className="fixed-book-booking-copy"><p className="fixed-book-kicker">Your idea starts here</p><h1>Turn the page.<br /><em>Start the piece.</em></h1><p>Share your idea, placement, rough size, budget and visual references. You will also need to confirm you are 18 or over.</p><div><button type="button" tabIndex={onBookingOpen ? 0 : -1} onClick={onBookingOpen}>Start an enquiry</button></div></div><div className="fixed-book-checklist"><p>Bring to the conversation</p><ol><li><span>01</span>Your idea</li><li><span>02</span>Placement</li><li><span>03</span>Rough size</li><li><span>04</span>Budget</li></ol></div><ReviewNote number={4} className="booking-review-low" /></>}
  </>;
}
  ...
  return <div className="booking-paper-overlay" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={dialogRef} className="booking-paper-dialog" role="dialog" aria-modal="true" aria-labelledby="booking-paper-title" aria-describedby="booking-paper-intro">
      ...
      <form className="booking-paper-form" onSubmit={sendEnquiry}>
        ...
        <label>Rough size<input name="roughSize" required placeholder="For example: 8cm or 3 inches" /></label>
        <label className="booking-paper-wide">Ideas for tattoo<textarea name="idea" required minLength={10} rows={4} placeholder="Describe the subject, style, details, and the feeling you want the piece to have…" /></label>
        <label>Placement<input name="placement" required placeholder="For example: inner forearm" /></label>
        <label>Budget<input name="budget" required placeholder="For example: £250" /></label>
        ...
        <label className="booking-paper-confirm booking-paper-wide"><input name="ageConfirmed" type="checkbox" value="yes" required /><span>I confirm I am 18 or over. Fletcher Tattoos may ask for proof of age before any appointment is agreed.</span></label>
